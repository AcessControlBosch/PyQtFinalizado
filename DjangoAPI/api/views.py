...
class GetAssociateAPIEDV(APIView):
    def get(self, request, pk="", pkid=""):
        if pk == '':
            associate = Associate.objects.all()
            serializer = GetAssociateTableEDV(associate, many=True)
            return Response(serializer.data)

        else:
            associate = Associate.objects.get(EDV=pk)
            serializer = GetAssociateTableEDV(associate)
            return Response(serializer.data)
    ...
    def patch(self, request, pk):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response({"msg": "Alterado com sucesso"})
#------------------------------------------------------------
#----------------------------Machine-------------------------
#------------------------------------------------------------
...
class GetReleaseMachineAnaliseAPI(APIView):
    def get(self, request, pk=''):

        if pk == '':
            releaseMachineTableResult = ReleaseMachine.objects.all()
            serializer = ReleaseMachineTableAnalise(releaseMachineTableResult, many=True)
            return Response(serializer.data)

        else:
            releaseMachineTableResult = ReleaseMachine.objects.filter(idAssociateFK=pk)
            serializer = ReleaseMachineTableAnalise(releaseMachineTableResult, many=True)
            return Response(serializer.data)
